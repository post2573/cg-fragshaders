...
void main() {
	...
	vec3 left = texture(image, vec2(texcoord.x - 1.0/width, texcoord.y)).rgb;
	vec3 right = texture(image, vec2(texcoord.x + 1.0/width, texcoord.y)).rgb;
	...
	vec3 sobel_h = bottom_right + (2.0 * right) + top_right - bottom_left - (2.0 * left) - top_left;
